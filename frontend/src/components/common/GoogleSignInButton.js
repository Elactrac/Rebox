...
const GoogleSignInButton = ({ 
  ...
  useEffect(() => {
    ...
    const checkOAuthConfig = async () => {
      try {
        ...
        setOauthEnabled(response.data?.data?.google?.enabled || false);
      } catch (error) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to initialize OAuth');
      }
    } catch (error) {
  ...
  if (!oauthEnabled) {
    return null; // Don't render if OAuth is not configured
  }
